...
EIOPA, the European Insurance and Occupational Pensions Authority, is a European Union financial regulatory institution. In this capacity it handles insurance and occupational pensions supervision in the EU. Besides supervision it also publishes tools and data which can be used by - among others - insurance companies. An example of data which is published (monthly) are the risk-free interest rate (RFR) term structures. The purpose of these term structures is to ensure a consistent calculation of technical provisions by companies across Europe.
"""

# ╔═╡ a5bc5425-0972-47cc-be22-9aeabf2345bb
md"""
In this notebook the pages on the EIOPA website which provide the risk-free interest rate term structures (current and previous versions) are scraped: zip-files containing Excel files with the risk-free interest rate term structures are identified by their name (date at the end of a month, *yyyymmdd*) and downloaded: consequently, the risk-free rate term structures is read from one of the Excel files in de zip-file and shown in a plot.
"""
...
md"""
Files with RFR term structures can be downloaded one by one as they are selected to be plotted, or they can be downloaded all at once using the checkbox below. Once files are downloaded, this local cache will be used when a specific RFR term structure is selected (although it is possible to download a file again and overwrite the local file).\
\
...
md"""
Select the number of years for which the interest rates should be shown. The minimum is 30 years, the maximum is 150 years:\
...
md"""
This section contains the code that actually scrapes the EIOPA website, processes the data and lets us analyze and plot the results. Underlying functions that are used can be found in the next section.
...
md"""
Specify the directories where downloaded zip-files and unzipped Excel files with RFR term structures will be saved:
"""
...
md"""
Select dates of risk-free interest rate term structures to be shown in the plot below (maximum of 4):\
